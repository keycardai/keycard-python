...
pip install git+ssh://git@github.com/stainless-sdks/keycard-api-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install keycard_api`
...
```sh
# install from this staging repo
pip install 'keycard_api[aiohttp] @ git+ssh://git@github.com/stainless-sdks/keycard-api-python.git'
```
...
client.with_options(max_retries=5).zones.list()
```
...
client.with_options(timeout=5.0).zones.list()
```
...
You can enable logging by setting the environment variable `KEYCARD_API_LOG` to `info`.

```shell
$ export KEYCARD_API_LOG=info
```
...
zone = response.parse()  # get the object that `zones.list()` would have returned
print(zone.items)
```

These methods return an [`APIResponse`](https://github.com/stainless-sdks/keycard-api-python/tree/main/src/keycard_api/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/keycard-api-python/tree/main/src/keycard_api/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/keycard-api-python/issues) with questions, bugs, or suggestions.
